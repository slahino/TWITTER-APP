...
class TweeterAuthentification extends \mf\auth\Authentification {
    ...
    public function createUser($username, $pass, $fullname, $level=self::ACCESS_LEVEL_USER) {
        ...
        if($user) {
            throw new \Exception('Username déjà existant.');
        }

        else {
            $nouvelUtilisateur = new tweeterapp\model\User();
            $nouvelUtilisateur->fullname = $fullname;
            $nouvelUtilisateur->username = $username;
            $nouvelUtilisateur->password = hashPassword($password);
            $nouvelUtilisateur->level = $level;
